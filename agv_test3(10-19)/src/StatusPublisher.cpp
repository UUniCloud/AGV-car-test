#include "StatusPublisher.h"//the 3st model//agv_test3//
...
{   
    ...
    car_status.encoder_ppr=84.2 * 84.2;//车轮1转对应的编码器个数
                    ...
                    if(mbUpdated)
                    {
                      for(j=0;j<7;j++)
                      {
                          if(cmd_string_buf[5*j+4]!=32)
                          {
                            ...
                            //   }
                            mbUpdated=false;
                            car_status.encoder_ppr=2000 * 84.2;
                            break;
                          }
                      }
                    }
...
void StatusPublisher::Refresh()//status refresh
{
    ...
    if(mbUpdated)
    {
      ...
      //pose
        double delta_car,delta_x,delta_y,var_len,var_angle,delta_t,delta_th,delta_distance=0.0;
        double l=1.020,d=0.075,vf=0.0,vv=0.0,vx=0.0,vy=0.0,alpha=0.0,theta=0.0,w=0.0;//m
        delta_t = 0.02;//second
        ...
        //theta为+向增大时，delta_car为正，v_steering为正；theta为负向增大时，delta_car为负，
        //v_steering为负；
        delta_distance = PI * 0.183f * delta_car /(2000 * 18.3f);//84.2f
        double v_steering = delta_distance * 50.0f;

        std::cout <<"delta_distance:!!!!!!!!!" << delta_distance << std::endl;
        std::cout <<"v_steering:!!!!!!!!!" << v_steering << std::endl;

        vf=  v_steering * cos(car_status.theta * PI / 180.0f);
        vv=  -v_steering * sin(car_status.theta * PI / 180.0f);//use vv to instead of tuoluoyi Wz
        w = vv / l;

        delta_th= w * delta_t;
        CarPos2D.theta +=delta_th;

        //distance decompose to Sf and Sv
        double Sf = delta_distance * cos(car_status.theta * PI / 180.0f);
        double Sv = delta_distance * sin(car_status.theta * PI / 180.0f);
        // Sf  decompose to delta_x and delta_y;
        if (v_steering != 0.0)
        {   // CarPos2D.theta is the initial and last angle.While car_status.theta is the current angle!!!!!!!!!!!!
            delta_x = Sf * cos(CarPos2D.theta * PI / 180.0f);
            delta_y = Sf * sin(CarPos2D.theta * PI / 180.0f);
        }
        else{
        ...
        CarPos2D.x +=delta_x;
        CarPos2D.y +=delta_y;
        //CarPos2D.theta +=delta_th;//发布的角度是the car during dt时间里转过的角度。
        ...
        */
        ...
        CarIMU.angular_velocity.x = -car_status.IMU[3]* PI /180.0f;//three angular speed
